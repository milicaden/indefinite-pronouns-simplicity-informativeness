...
# Pragmatic speaker
def s1(state, word, df):
    temp = df[df['ITEM'] == word]
    language = df[df['LANG'] == temp['LANG'].values[0]]
    denominator = 0
    for i in language['ITEM']:
        k = math.exp(log_score(state, i, df))
        denominator = denominator + k
    numerator = math.exp(log_score(state, word, df))
    if denominator == 0:
        return 0 #this step to assign probability of each item given some flavor x to be 0 if no item can express that flavor in the language
    else:
...
# Pragmatic listener
def l1(state, word, df):
  numerator = s1(state, word, df)*probaf(state)
  denominator = 0
  for i in ["sk", "su", "ns", "npi", "fci", "nq"]:
...
def RSAcost(language, df):
    if RSAutility(language, df) == 0:
        return np.nan
    else:
        return 1/RSAutility(language, df)
...
def RSAcostcomplexity(data_frame):
    ...
    RSAcostoflanguages = []
    for i in languages:
        k = RSAcost(i, data_frame)
        RSAcostoflanguages.append(k)

    all = pd.DataFrame(list(zip(languages, complexityoflanguages, RSAcostoflanguages)),
                       columns=['LANG', 'complexityoflanguages', 'RSAcostoflanguages'])
...
if __name__ == "__main__":
    ...
    mindesc = pd.read_csv(Folder + "minimum-desc-indef-with-compl.csv", delimiter = ";") #file with complexity information for each item (based on their shortest descriptions)
    ...
    for i in range(1,num_of_gen+1):
        temp = copy(languages)
        temp = prep(temp)
        tempcc = RSAcostcomplexity(temp)
        tempcc['new_col'] = list(zip(tempcc.RSAcostoflanguages, tempcc.complexityoflanguages))
        dominating_indices = pygmo.non_dominated_front_2d(points = list(tempcc['new_col']))
    ...
    costcomlang = RSAcostcomplexity(languages)
    costcomlang.to_csv(r''+Folder+'RSAfinalgencostcom.csv', index=False)
    ...
    allfinaluntyped = allfinal.drop(columns = ['type','costoflanguages'])
    domselect = pd.concat([costcomlang, allfinaluntyped])
    domselect.reset_index(inplace=True, drop=True)

    domselect['new_col'] = list(zip(domselect.RSAcostoflanguages, domselect.complexityoflanguages))
    dominating_indices = pygmo.non_dominated_front_2d(points = list(domselect['new_col']))
    ...
    domselect.to_csv(r''+Folder+'RSApareto_dominant.csv', index=False)
